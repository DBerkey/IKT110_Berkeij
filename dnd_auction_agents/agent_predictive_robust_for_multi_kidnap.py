...
def predictive_bidding(agent_id: str, current_round: int, states: Dict[str, Dict[str, Any]], 
    ...
    base_bid = 15  # Slightly lower than tiny_bid's 20 minimum
    max_bid = 80   # Higher than tiny_bid's default 20
    ...
    if random_int <= 5 and current_gold > 20000: 
        # Bid on auctions 10-15 instead of top auctions
        all_auction_ids = list(auctions.keys())
        auctions_10_to_15 = all_auction_ids[10:16]  # Get auctions 10-15 (indices 10-15)
        if auctions_10_to_15:
            chosen_auction = random.choice(auctions_10_to_15)
        ...
        # Get all auction IDs and skip the top 10
        all_auction_ids = list(auctions.keys())
        auctions_to_consider = all_auction_ids[10:]  # Skip first 10 auctions
        
        for auction_id in auctions_to_consider:
