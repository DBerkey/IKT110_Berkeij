...
class AuctionAgent:
    def __init__(
        ...
        self.params = {
            ...
            "bid_coverage": 0.2,
            "min_bid": 1,
            "pool_policy": "conservative",
            "adaptive": False,  # disable adaptive updates
            "pool_min_gpp_multiplier": 1.5,
            "max_points_fraction_per_round": 0.1,
            "min_point_reserve": 2,
            "pool_required_win_rate": 0.1,
        }
        ...
        # ML model for aggressiveness (online learning)
        self.ml_model = SGDRegressor(
            max_iter=1, learning_rate="constant", eta0=0.01, warm_start=True
        )
        self.ml_X = []
        self.ml_y = []
    ...
    def predict_aggressiveness(self, gold, points, round_no):
        x = np.array([[gold, points, round_no / 1000]])
        if self.ml_trained:
            pred = self.ml_model.predict(x)[0]
            pred *= np.random.uniform(0.9, 1.1)
            return max(0.2, min(2.5, pred))
        else:
            return self.params["aggressiveness"]

    def update_ml(self, gold, points, round_no, reward):
        if gold == 0:
            return
        X_new = np.array([[gold, points, round_no / 1000]])
        y_new = np.array([reward])
        self.ml_X.append(X_new[0])
        self.ml_y.append(y_new[0])
        if len(self.ml_X) >= 5:
            self.ml_model.partial_fit(np.array(self.ml_X), np.array(self.ml_y))
    ...
    def make_bid(
        ...
        self.params["aggressiveness"] = self.predict_aggressiveness(
            gold, points, round_no
        )

        # Stage-based spending
        if round_no <= 400:  # Early: tiny spend, only top auctions
            max_spend = max(1, int(gold * 0.03))
            bid_coverage = 0.1
        elif round_no <= 800:  # Mid: moderate spend
            max_spend = max(1, int(gold * 0.3))
            bid_coverage = 0.3
        else:  # Late: aggressive
            max_spend = gold
            bid_coverage = self.params["bid_coverage"]
        ...
        num_to_consider = max(1, int(len(ev_list) * bid_coverage))
    ...
    def update_from_round_results(
        ...
        reward = points_gained / max(1, gold_spent)
        self.update_ml(rec["state"]["gold"], rec["state"]["points"], round_no, reward)
