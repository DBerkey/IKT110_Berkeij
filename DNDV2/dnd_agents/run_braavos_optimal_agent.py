#!/usr/bin/env python3
"""Launcher for the BraavosOptimalAgent."""
...
def _load_agent_class():
    ...
    try:
        from dnd_agents.BraavosOptimalAgent import BraavosOptimalAgent  # type: ignore
    except ImportError as exc:  # pragma: no cover - runtime guard
        raise RuntimeError(
            "Unable to import BraavosOptimalAgent. Ensure the project root is on PYTHONPATH."
        ) from exc
    return BraavosOptimalAgent
...
def run_agent(host: str, port: int, agent_name: str, player_id: str) -> None:
    ...
    try:
        ...
        print("<BraavosOptimalAgent interrupted>")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the BraavosOptimalAgent")
    ...
    parser.add_argument(
        "--agent-name",
        default="braavos_optimal_agent",
        help="Display name reported to the server",
    )
    parser.add_argument(
        "--player-id",
        default="braavos_optimal_player",
        help="Player id used in the log for tracking",
    )
