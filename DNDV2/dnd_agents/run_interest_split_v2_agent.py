#!/usr/bin/env python3
"""Launcher for the InterestSplitV2Agent."""
...
def _load_agent_class():
    ...
    try:
        from dnd_agents.InterestSplitV2Agent import InterestSplitV2Agent  # type: ignore
    except ImportError as exc:  # pragma: no cover
        raise RuntimeError(
            "Unable to import InterestSplitV2Agent. Ensure the project root is on PYTHONPATH."
        ) from exc
    return InterestSplitV2Agent
...
def run_agent(host: str, port: int, agent_name: str, player_id: str) -> None:
    ...
    try:
        ...
        print("<InterestSplitV2Agent interrupted>")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the InterestSplitV2Agent")
    ...
    parser.add_argument(
        "--player-id",
        default="interest_split_player",
        help="Player id recorded in server logs",
    )
...
def main() -> None:
    args = parse_args()
    default_name = f"interest_split_v2_{random.randint(1, 999_999)}"
    agent_name = args.agent_name or default_name
    run_agent(args.host, args.port, agent_name, args.player_id)
