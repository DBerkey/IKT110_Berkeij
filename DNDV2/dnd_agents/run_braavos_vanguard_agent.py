#!/usr/bin/env python3
"""Launcher for the BraavosVanguardAgent."""
...
def _load_agent_class():
    ...
    try:
        from dnd_agents.BraavosVanguardAgent import BraavosVanguardAgent  # type: ignore
    except ImportError as exc:  # pragma: no cover - runtime guard
        raise RuntimeError(
            "Unable to import BraavosVanguardAgent. Ensure the project root is on PYTHONPATH."
        ) from exc
    return BraavosVanguardAgent
...
def run_agent(host: str, port: int, agent_name: str, player_id: str) -> None:
    ...
    try:
        ...
        print("<BraavosVanguardAgent interrupted>")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the BraavosVanguardAgent")
    ...
    parser.add_argument(
        "--agent-name",
        default="braavos_vanguard_agent",
        help="Display name reported to the server",
    )
    parser.add_argument(
        "--player-id",
        default="braavos_vanguard_player",
