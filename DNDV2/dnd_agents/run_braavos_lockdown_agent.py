#!/usr/bin/env python3
"""Launcher for the BraavosLockdownAgent."""
...
def _load_agent_class():
    ...
    try:
        from dnd_agents.BraavosLockdownAgent import BraavosLockdownAgent  # type: ignore
    except ImportError as exc:  # pragma: no cover
        raise RuntimeError(
            "Unable to import BraavosLockdownAgent. Ensure the project root is on PYTHONPATH."
        ) from exc
    return BraavosLockdownAgent
...
def run_agent(host: str, port: int, agent_name: str, player_id: str) -> None:
    ...
    try:
        ...
        print("<BraavosLockdownAgent interrupted>")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the BraavosLockdownAgent")
    ...
    parser.add_argument("--agent-name", default="braavos_lockdown_agent", help="Display name for the agent")
    parser.add_argument(
        "--player-id",
        default="braavos_lockdown_player",
