#!/usr/bin/env python3
"""Launcher for the InterestSplitAgent."""
...
def _load_agent_class():
    ...
    try:
        from dnd_agents.InterestSplitAgent import InterestSplitAgent  # type: ignore
    except ImportError as exc:  # pragma: no cover
        raise RuntimeError(
            "Unable to import InterestSplitAgent. Ensure the project root is on PYTHONPATH."
        ) from exc
    return InterestSplitAgent
...
def run_agent(host: str, port: int, agent_name: str, player_id: str) -> None:
    ...
    try:
        ...
        print("<InterestSplitAgent interrupted>")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the InterestSplitAgent")
...
def main() -> None:
    args = parse_args()
    agent_name = args.agent_name or f"interest_split_{random.randint(1, 999_999)}"
