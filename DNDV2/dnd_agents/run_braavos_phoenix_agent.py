#!/usr/bin/env python3
"""Launcher for the BraavosPhoenixAgent."""
...
def _load_agent_class():
    ...
    try:
        from dnd_agents.BraavosPhoenixAgent import BraavosPhoenixAgent  # type: ignore
    except ImportError as exc:  # pragma: no cover
        raise RuntimeError(
            "Unable to import BraavosPhoenixAgent. Ensure the project root is on PYTHONPATH."
        ) from exc
    return BraavosPhoenixAgent
...
def run_agent(host: str, port: int, agent_name: str, player_id: str) -> None:
    ...
    try:
        ...
        print("<BraavosPhoenixAgent interrupted>")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the BraavosPhoenixAgent")
    ...
    parser.add_argument(
        "--agent-name",
        default="braavos_phoenix_agent",
        help="Display name used on the leaderboard",
    )
    parser.add_argument(
        "--player-id",
        default="braavos_phoenix_player",
