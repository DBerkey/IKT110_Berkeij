#!/usr/bin/env python3
"""Launcher for the BraavosSentinelAgent."""
...
def _load_agent_class():
    ...
    try:
        from dnd_agents.BraavosSentinelAgent import BraavosSentinelAgent  # type: ignore
    except ImportError as exc:  # pragma: no cover - runtime guard
        raise RuntimeError(
            "Unable to import BraavosSentinelAgent. Ensure the project root is on PYTHONPATH."
        ) from exc
    return BraavosSentinelAgent
...
def run_agent(host: str, port: int, agent_name: str, player_id: str) -> None:
    ...
    try:
        ...
        print("<BraavosSentinelAgent interrupted>")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the BraavosSentinelAgent")
    ...
    parser.add_argument(
        "--agent-name",
        default="braavos_sentinel_agent",
        help="Display name reported to the server",
    )
    parser.add_argument(
        "--player-id",
        default="braavos_sentinel_player",
