#!/usr/bin/env python3
"""Launcher for the BraavosLegionAgent."""
...
def _load_agent_class():
    ...
    try:
        from dnd_agents.BraavosLegionAgent import BraavosLegionAgent  # type: ignore
    except ImportError as exc:  # pragma: no cover
        raise RuntimeError(
            "Unable to import BraavosLegionAgent. Ensure the project root is on PYTHONPATH."
        ) from exc
    return BraavosLegionAgent
...
def run_agent(host: str, port: int, agent_name: str, player_id: str) -> None:
    ...
    try:
        ...
        print("<BraavosLegionAgent interrupted>")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the BraavosLegionAgent")
    ...
    parser.add_argument(
        "--agent-name",
        default="braavos_legion_agent",
        help="Display name used on the leaderboard",
    )
    parser.add_argument(
        "--player-id",
        default="braavos_legion_player",
