#!/usr/bin/env python3
"""Launcher for the BraavosCenturionAgent."""
...
def _load_agent_class():
    ...
    try:
        from dnd_agents.BraavosCenturionAgent import BraavosCenturionAgent  # type: ignore
    except ImportError as exc:  # pragma: no cover
        raise RuntimeError(
            "Unable to import BraavosCenturionAgent. Ensure the project root is on PYTHONPATH."
        ) from exc
    return BraavosCenturionAgent
...
def run_agent(host: str, port: int, agent_name: str, player_id: str) -> None:
    ...
    try:
        ...
        print("<BraavosCenturionAgent interrupted>")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the BraavosCenturionAgent")
    ...
    parser.add_argument(
        "--agent-name",
        default="braavos_centurion_agent",
        help="Display name used on the leaderboard",
    )
    parser.add_argument(
        "--player-id",
        default="braavos_centurion_player",
